# This script produces a single summary table that can be used for later analysis. It
# requires the availability of a number of datafiles in the working directory (see below).
# These datafiles contain data collected from the accelerometers from the Samsung Galaxy S smartphone.
# The summary table named "total_set_mean" gives some summary data per testperson, per activity type.
# 
# First, read in all data tables. The relevant tables are named in accordance with   
...
#
# Now we remove the ‘ Id’-column - it is not needed any more. And we change the order 
# of the columns in the table, so that Subject and Activity come first.
#
...
#
# Again change the order of the columns, because the merge-action put them in an     
# order we don’t want.
#
...
# activity and each subject. I used ‘Group by’ and ‘summarise’ to realise this. And  
# changed the names of the columns, so that they reflect a difference with the       
# previous data set. The name of this data set is total_set_mean.  
#
